...
public class ProverImplFociVampyre implements Prover {
	...
	public static Prover getInstance() {
		if (prover == null)
			prover = new ProverImplFociVampyre();
		return (Prover) prover;
	}

	/*
	 * public Set<String> doSubstitution(Set<EdgeLabel> edges){ //for
	 * evaluation sentence v=e; //e is substituted by former regulation; //and
	 * generate new rule for v;
	 *  }
	 */

	// if the edges are satisfiable, throw Exception
	public List<Predicate> getInterpolation(List<EdgeLabel> edges)
			throws Exception {
		...
		BufferedReader inputReader = new BufferedReader(new InputStreamReader(
				fociProcess.getInputStream()));
		BufferedReader errorReader = new BufferedReader(new InputStreamReader(
				fociProcess.getErrorStream()));
		...
		for (AdvCondition ac : interpolations) {
			// if(debugging)System.out.println("Getting predicate:
			// "+ac.toString());
			// filter the true conditions: we don't need true to be our
			// predicate.
			if (!ac.isTrue(this) && !ac.isFalse()) {
				Predicate p = new Predicate(ac);
				predicates.add(p);
				// System.out.println("Now predicate number: "+
				// predicates.size());
			}
		}
	...
	public boolean isSatisfiable(List<AdvCondition> clist) {
		...
		try {
			...
			BufferedReader inputReader = new BufferedReader(
					new InputStreamReader(fociProcess.getInputStream()));
			BufferedReader errorReader = new BufferedReader(
					new InputStreamReader(fociProcess.getErrorStream()));
					...
					if (tok == st.TT_EOF)
						...
						if (tok == st.TT_WORD) {
							...
							if (st.sval.equals("Fatal")) {
								System.out
										.println("Warning: error occurred. There might be syntax error! The answer is regarded as false.");
								result = false;
								streamEnd = true;
								gotresult = true;
			...
			if (!gotresult) {
				System.out
						.println("Warning: No useful signals received. The answer is regarded as true.");
				return true;
			}
		} catch (Exception e) {
			System.err.println(e);
			System.out
					.println("Warning: Exception caught. The answer is regarded as false.");
			return false;
		}
		return result;
	}
	...
	private boolean implyByVampyre(String c1lf, String c2lf) {
		...
		try {
			...
			System.err
					.println("Warning: Error occurred when writing temp file. The answer is regarded as false.");
			return false;
		}
		...
		try {
			...
			while (!streamEnd) {
				...
				if (tok == st.TT_EOF)
					...
					if (tok == st.TT_WORD) {
						...
						/*
						 * else if (st.sval.equals("Error")){ result=false;
						 * streamEnd=true; gotresult=true; }
						 */
					}
				}
			}
			...
			br = new BufferedReader(new InputStreamReader(vampyreProcess
					.getErrorStream()));
			...
			while (!streamEnd) {
				...
				if (tok == st.TT_EOF)
					...
					if (tok == st.TT_WORD) {
						// System.out.println(st.sval);
						/*
						 * if(st.sval.equals("successfully")){ result=true;
						 * streamEnd=true; gotresult=true; }
						 */
						if (st.sval.equals("Error")) {
							result = false;
							System.err
									.println("Vampyre reported error: there might be syntax error(s). The answer is regarded as false.");
							streamEnd = true;
							gotresult = true;
						}
					}
			...
			System.err
					.println("Error occurred when excuting vampyre. The answer is regarded as false.");
			return false;
		}
		if (!gotresult) {
			// throw new Exception("result is null when invoking
			// implyByVampyre");
			// System.err.println("Warning: vampyre engine failed. The answer is
			// regarded as false.");
			return false;
		}
		return result;
